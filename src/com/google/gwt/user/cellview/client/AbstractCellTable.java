    ...
    if (targetTableSection == thead || targetTableSection == tfoot) {
      ...
      if (headerParent != null) {
        ...
        if (header != null) {
          ...
          Element filterImageElement = getImageElement(headerParent);

          // Check if click on filter image element
          if (event.getEventTarget().equals(filterImageElement)) {
              // Check if firefox and cancel sort on this header
              if (isSelect && Window.Navigator.getAppName().equals("Netscape")) {
                  isSelect = false;
              }
          }
